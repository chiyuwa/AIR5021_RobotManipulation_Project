...
/*        pTest = new sdk_sagittarius_arm::CSDarmCommonSerial("/dev/ttyUSB0", "115200", iTimeLimit, nh, pnh); //just for test
    ...
    void SagittariusArmReal::JointStatesCb(const sensor_msgs::JointState& cmd_arm)
    {
        if(pSDKarm!=NULL)
        {
            if (cmd_arm.position.size() < 7)
            {
                ROS_WARN_THROTTLE(2.0, "JointStatesCb received only %zu positions, expected at least 7.", cmd_arm.position.size());
                return;
            }
    ...
    void SagittariusArmReal::arm_joint_trajectory_msg_callback(const trajectory_msgs::JointTrajectory &msg)
    {
        if (execute_joint_traj == false)
        {
            ...
            for (auto const& joint:arm_joints)
            {
                if (joint.name != "joint_gripper_left")
                {
                    if (itr >= joint_states.position.size() || itr >= joint_states.name.size())
                    {
                        ROS_WARN("Arm initial-state check skipped because driver joint_states is incomplete.");
                        break;
                    }
    ...
    void SagittariusArmReal::arm_gripper_trajectory_msg_callback(const trajectory_msgs::JointTrajectory &msg)
    {
        if (execute_gripper_traj == false)
        {
            ...
            gripper_tra_msg = msg;
            joint_states = pSDKarm->joint_states;
            if (joint_states.position.size() <= arm_joints.size())
            {
                ROS_WARN("Gripper initial-state check skipped because driver joint_states is incomplete.");
            }
            else if (!(fabs(gripper_tra_msg.points.at(0).positions.at(0) - joint_states.position.at(arm_joints.size())) < 0.1))
            {
