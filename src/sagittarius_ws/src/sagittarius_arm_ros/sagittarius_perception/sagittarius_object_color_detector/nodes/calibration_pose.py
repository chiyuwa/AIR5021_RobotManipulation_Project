...
def talker(threadName, delay):
    ...
    for i in range(len(point_list)):
        ...
        while(next_cali_pos == 0):
            ...
            cnt += 1
            if cnt > 50:
                rospy.logerr("timeout waiting \"next\" command from calibration_node. Please check whether calibration.py received the start command, whether /usb_cam/image_raw is publishing, and whether the blue block is visible.")
                break
        if(next_cali_pos == 0):
            return
        next_cali_pos = 0
